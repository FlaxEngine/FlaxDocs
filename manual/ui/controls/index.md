# Controls

Games created with Flax build UI using **controls**. Each control is an independant object and implements only the most basic conepts like transformation, background color, and visibility. The more specific logic like click event, text drawing, child controls layout can be achived using a specific control type. Flax Engine offers a solid set of ready to use controls that can be used in a games GUI. To create your own control types see a related tutorial [here](../tutorials/create-custom-control.md).

# Default control types

* [Button](button.md)
* [CheckBox](checkbox.md)
* [Image](image.md)
* [Label](label.md)
* [Dropdown](dropdown.md)
* [TextBox](textbox.md)
* [Border](border.md)
* [Panel](panel.md)
* [Drop Panel](drop-panel.md)
* [Blur Panel](blur-panel.md)
* [Progress Bar](progress-bar.md)
* [Spacer](spacer.md)
* [Grid Panel](grid-panel.md)
* [Uniform Grid Panel](uniform-grid-panel.md)
* [Horizontal Panel](horizontal-panel.md)
* [Vertical Panel](vertical-panel.md)
* [Tiles Panel](tiles-panel.md)
